...
class FeatureEngineeringAgent:
    ...
    def _add_context_features(self, df: pd.DataFrame) -> pd.DataFrame:
        df["traffic_index"] = df.get("traffic_index", 0).fillna(0)
        df["industrial_score"] = df.get("industrial_score", 0).fillna(0)
        df["green_cover"] = df.get("green_cover", 0.0).fillna(0.0)
        return df

    def _add_health_risk(self, df: pd.DataFrame) -> pd.DataFrame:
        df["health_risk"] = df["aqi"].apply(lambda value: "High" if value >= 150 else "Moderate" if value >= 100 else "Low")
        return df
